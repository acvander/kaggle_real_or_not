...
def plot_k_fold_data(histories: List[tf.keras.callbacks.History],
    ...
    for i, history in enumerate(histories):
        ...
        axs[0].plot(loss, colors[i], alpha=0.7)
        axs[0].set_title('Loss')
        axs[0].set_ylim(bottom=0, top=1)
        axs[1].plot(val_loss, colors[i], alpha=0.7)
        axs[1].set_title('Val Loss')
        axs[1].set_ylim(bottom=0, top=1)
        axs[2].plot(f1_score, colors[i], alpha=0.7)
        axs[2].set_title('F1 Score')
        axs[2].set_ylim(bottom=0.5, top=1)
        axs[3].plot(val_f1_score, colors[i], alpha=0.7)
